...
- **Choice:** A `BuildHookInterface` subclass in `hatch_build.py` reads `MCP_BRAND={hydrolix|trafficpeak}` (default `hydrolix`) and mutates `project.name`, `[project.scripts]`, `project.urls`, author/maintainer/description, and bakes `__brand__` into `mcp_hydrolix/_brand.py` inside the wheel. The hook also reads this repo's `README.md`, applies a brand-substitution filter to its contents, and uses the result as the wheel's `long_description`. The filter is a **case-insensitive, case-preserving** replacement of every `hydrolix` occurrence with the target brand (so `mcp-hydrolix`, `HYDROLIX_`, `Hydrolix`, `hydrolix.live`, `mcp--hydrolix` badges, and lowercase `hydrolix_*` config keys all rebrand). Customer-facing prose MUST NOT mention the other brand; the hydrolix org that legitimately appears in repository URLs is tolerated ONLY in structured manifest/metadata fields (the `BRANDS` urls table, `mcpb/build.sh`), never in this prose filter. The GitHub-org identity namespaces `io.github.hydrolix` and `github.com/hydrolix` are additionally exempt from the scrub itself (they name the owning org, not the brand) -- the package segment after them still rebrands (`io.github.hydrolix/mcp-trafficpeak`). In Hydrolix mode the filter is an identity transform. This way the `mcp-trafficpeak` PyPI landing page is generated from this repo's single-source `README.md` at publish time rather than being maintained as a vendored file in the sibling repo (see Decision: tp-pypi-via-sibling-repo).
...
- **Implementation note (name cannot be dynamic under uv):** `uv` / PEP 621 require a *static* `project.name` — `dynamic = ["name"]` is rejected by the uv frontend ("project.name must be present"), a `{env:...}` context value fails uv's name validation, and a metadata hook does not override a statically-present name. Renaming the artifact inside the build backend also fails, because a PEP 517 frontend verifies that the filename the backend *returns* exists on disk. Therefore only the fields that CAN be dynamic (`description`, `scripts`, `urls`, `readme`) are set by the metadata hook per brand; the `mcp-trafficpeak` *rename* (`.dist-info` dir, `Name` field / `PKG-INFO`, `RECORD`, artifact filename) is applied **out-of-backend** by the wrapper `scripts/brand_build.py`, which builds into a private temp dir and moves the renamed artifacts out (so it never clobbers a paired Hydrolix build). The TP build command is thus `MCP_BRAND=trafficpeak python scripts/brand_build.py` (a transparent `uv build` passthrough for the Hydrolix brand), and the sibling repo's publish workflow invokes the wrapper rather than bare `uv build`.

### Decision: dual-namespace-precedence

- **Choice:** Resolve **per-variable at read time**, without mutating the environment. A helper `brand_getenv(name)` returns, for each canonical `HYDROLIX_<suffix>`, the `TRAFFICPEAK_<suffix>` value when set, else the `HYDROLIX_<suffix>` value (silent fallback); `HydrolixConfig` reads every variable through it. No `HYDROLIX_*` is deleted, nothing is projected into `os.environ`, and no conflict warning is emitted. Deprecated-alias suffixes (`HOST`/`PORT`/`SECURE`/`API_HOST`/`API_PORT`/`PROXY_PATH`) are Hydrolix-only and never mirrored. If neither namespace supplies a URL, exit non-zero naming **only the running brand's own anchor** (`TRAFFICPEAK_URL` for a `mcp-trafficpeak` build, `HYDROLIX_URL` for a `mcp-hydrolix` build).
- **Why:** Per-variable prefer-TP with silent HYDROLIX fallback lets a deployment set operational defaults once — under `HYDROLIX_*`, e.g. a container image's `HYDROLIX_MCP_SERVER_TRANSPORT`/`HYDROLIX_MCP_BIND_HOST` — and have them survive when a customer supplies only `TRAFFICPEAK_URL`.
- **Reversed from whole-chain:** An earlier version used whole-chain semantics ("if `TRAFFICPEAK_URL` is set, delete every `HYDROLIX_*` and use TP entirely; warn on conflict"), implemented by mutating `os.environ`. That was reversed twice: first the semantics (whole-chain wiped operational defaults — a TrafficPeak container given only `TRAFFICPEAK_URL` silently reverted to stdio/loopback), then the mechanism (env mutation replaced by the read-time `brand_getenv` coalesce, which avoids process-global side effects and cross-process leakage).
- **Trade-off:** Per-variable resolution permits a mixed configuration (e.g. `TRAFFICPEAK_URL` alongside an inherited `HYDROLIX_HTTP_QUERY_HOST` pointing elsewhere). Accepted: real deployments set one namespace for connection config, and operational-default survival outweighs guarding that edge case.
- **Advertising:** Which prefix the server *names* in user-facing output (connection-target error hint, docs, mcpb config keys) is chosen by the baked brand — a `mcp-trafficpeak` build advertises `TRAFFICPEAK_*`, a `mcp-hydrolix` build advertises `HYDROLIX_*` — even though both builds *accept* both prefixes.
- **Binding:** Resolution MUST be a per-variable read-time coalesce (TP over HYDROLIX) via `brand_getenv()`; it MUST NOT mutate `os.environ`, MUST NOT delete `HYDROLIX_*` variables, and MUST NOT emit a cross-namespace conflict warning. The connection-target error MUST name only the running brand's own anchor.
