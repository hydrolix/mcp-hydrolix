...
The OAuth 2.1 prototype on `il/feature/oauth-support/hdx-11133` covers the intended shape — RS256 verifier, OIDC discovery, fail-open startup, fail-closed request-time, RFC 9728 metadata, in-cluster JWKS backchannel, SA-credential fallback — with 8 test files. But it is investigation-grade: the tests haven't been re-verified against current `main` deps, the security checklist isn't signed off, and end-to-end validation against the production IdP (a cluster-deployed OIDC proxy being built concurrently by the turbine-API team under [HDX-11431](https://hydrolix.atlassian.net/browse/HDX-11431)) hasn't happened.

It also predates the gunicorn→uvicorn migration ([HDX-10675](https://hydrolix.atlassian.net/browse/HDX-10675)). The prototype mutates `mcp.auth` once in the Gunicorn supervisor — but on `main`, uvicorn spawns workers that re-import `webapp.py:create_app` from scratch, so supervisor-side mutations don't propagate. The activation path is structurally broken on `main`.

Shipping this is the prerequisite for the cluster-served auth story: the in-cluster mcp-hydrolix pod behind Traefik at `/mcp`, with the cluster-deployed IdP proxy issuing bearers. The verifier is OIDC-agnostic by construction (plain discovery + JWKS), so this change can land in parallel with [HDX-11431](https://hydrolix.atlassian.net/browse/HDX-11431).
...
- Audit `logger.*` call sites in `oauth.py`, `mcp_providers.py`, `mcp_server.py`, `webapp.py` against the spec's credential-leak contract: raw JWT, signature segment, base64url header/payload segments, full `Authorization` header value, and JWKS key material are excluded from logs.
- Port `docs/oauth.md`: updated for the uvicorn entrypoint, the `mcp-hydrolix,config-api` audience example, and the 16-row [HDX-11133](https://hydrolix.atlassian.net/browse/HDX-11133) security checklist verbatim with per-row sign-off or carve-out annotations.
...
**Two-track structure** (detail in design.md): Track 1 is "code implementation and IdP-agnosticism except for `canonical_idp_endpoints`"; Track 2 is "rollout and validation gates target [HDX-11431](https://hydrolix.atlassian.net/browse/HDX-11431)".
...
- **Related work**: [HDX-11441](https://hydrolix.atlassian.net/browse/HDX-11441) (`HYDROLIX_URL`) has spec PR [#101](https://github.com/hydrolix/mcp-hydrolix/pull/101) open; runtime PR not yet open. Sequencing assumes [HDX-11441](https://hydrolix.atlassian.net/browse/HDX-11441) lands first — a single canonical host URL collapses several activation-gate and audience-derivation branches here, which is reflected in the stage count and the `canonical_idp_endpoints` shape below.
