...
- Is the HDX-11133 plan doc (`oauth-support-hdx-11133.md`) checked in anywhere? It does not appear on the prototype branch (Explore confirmed via `git ls-tree`). The 16-row security checklist referenced by the ticket AC needs a canonical location — either re-attach the doc to the ticket, copy section 4 verbatim into `docs/oauth.md` as a "Security checklist (HDX-11133 section 4)" subsection, or accept the ticket comment as the sign-off artifact. Decision needed before "checklist sign-off" can be a meaningful gate.
