...
- [ ] 1.4 Audit `webapp.py` log calls: confirm `_activate_oauth_if_configured` logs only the resolved issuer URL, audience, and required scopes — not the raw JWKS or discovery responses [implements: oauth-authentication/no-jwt-credential-material-in-logs, design/log-content-is-tested-invariant] — verify: `grep -n "logger\." mcp_hydrolix/webapp.py` shows no raw discovery/JWKS body references
...
- [ ] 3.2 Run full test suite to confirm no regressions from the log-call changes in phase 1 [implements: oauth-authentication/no-jwt-credential-material-in-logs, meta/tests] — verify: `uv run pytest` exits 0
