...
- **Choice**: When every backend in the chain defers (`None`) for a bearer-bearing request, the chain owner SHALL emit exactly one WARNING-level log line — including the unverified `iss` claim from the JWT, the configured `OAuthConfig.issuer`, and a fixed greppable failure-mode phrase — before returning 401. Per-backend deferrals stay silent; the WARNING is emitted only at the boundary where the all-defer outcome is observable. Tokens that are claimed by *some* backend (even if that backend then rejects them) do not trigger the WARNING.
- **Why**: An `iss` matching neither the OAuth IdP nor the SA shape almost always indicates a deployment problem (wrong IdP URL, stale realm after rotation, audience renamed, SA suffix changed) rather than an unauthorized login attempt. Surfacing it loudly at WARNING level keeps it visible to operators in log aggregation while not paging anyone — the worker still rejected the request correctly. Routing it through the chain owner (rather than each backend) avoids N log lines per request, prevents drift if backends are added or reordered, and is the only site where "no one claimed this" is observable.
- **Alternatives**:
