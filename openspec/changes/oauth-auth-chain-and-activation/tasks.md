...
- [ ] 3.10 `create_app()` under active event loop raises `RuntimeError` pointing at test setup [implements: design/asyncio-run-inside-create-app, meta/tests] — verify: `pytest -q tests/test_oauth_auth_chain_and_activation.py::test_create_app_under_active_loop_raises` green

- [ ] 3.11a Register the `unrecognized_issuer_total` counter in `mcp_hydrolix/metrics.py`: add a field to the `Metrics` dataclass (`unrecognized_issuer_total: _PidGuardedCounter | _NoOpMetric`), add the corresponding `Counter("unrecognized_issuer_total", "Bearer tokens whose iss matched no chain backend", namespace=NAMESPACE)` entry in `_build_live()`, and add `_NOOP` for it in `_NOOP_METRICS`. No labels [implements: oauth-authentication/unrecognized-issuer-surfaces-as-deployment-warning, design/unrecognized-issuer-warning-at-chain-boundary] — verify: `ruff check mcp_hydrolix/metrics.py` clean and `mypy mcp_hydrolix/metrics.py` passes

- [ ] 3.11b Implement the chain-boundary unrecognized-issuer WARNING and metric: wrap the chain dispatch (in `HydrolixCredentialChain.get_middleware()` or an adjacent helper called from `mcp.auth`) so that when all backends in the chain return `None` for a bearer-bearing request, the wrapper (i) emits one WARNING-level log line containing the unverified `iss` claim, `OAuthConfig.issuer`, and the fixed phrase `"bearer iss matched no chain backend — likely IdP misconfiguration"`, (ii) calls `METRICS.unrecognized_issuer_total.inc()`, then returns 401. No-bearer requests SHALL NOT trigger this path. The wrapper SHALL be the sole emitter — individual backends remain silent on their own deferrals [implements: oauth-authentication/unrecognized-issuer-surfaces-as-deployment-warning, design/unrecognized-issuer-warning-at-chain-boundary] — verify: `ruff check mcp_hydrolix/auth/mcp_providers.py` clean

- [ ] 3.12 "Bearer With Unknown Issuer Yields 401, WARNING, And Metric Increment": OAuth active with metrics enabled, bearer whose `iss` matches neither `OAuthConfig.issuer` nor the SA `iss` shape; assert 401 + exactly one WARNING-level log record containing both the unverified `iss` and the configured `OAuthConfig.issuer`; assert the record contains no raw JWT or base64url segments (cross-check with `oauth-log-redaction`'s caplog helper if available); assert `unrecognized_issuer_total` incremented by exactly 1 by sampling the counter via `prometheus_client` before and after the request [implements: oauth-authentication/unrecognized-issuer-surfaces-as-deployment-warning, meta/tests] — verify: `pytest -q tests/test_oauth_auth_chain_and_activation.py::test_bearer_with_unknown_issuer_yields_401_warning_and_metric_increment` green

- [ ] 3.13 "Routine Deferrals Stay Silent": OAuth active with metrics enabled, valid SA bearer (defers from OAuth verifier, claimed by `BearerAuthBackend`); assert no WARNING-level "unrecognized issuer" log record is emitted; assert `unrecognized_issuer_total` is unchanged (sampled before/after) [implements: oauth-authentication/unrecognized-issuer-surfaces-as-deployment-warning, meta/tests] — verify: `pytest -q tests/test_oauth_auth_chain_and_activation.py::test_routine_deferrals_stay_silent` green
