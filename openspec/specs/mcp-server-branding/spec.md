# mcp-server-branding Specification

## Purpose
Defines how the local MCP server's distribution name, console-script, runtime brand identifier, env-var contract, User-Agent, and release artifacts are determined per brand (`mcp-hydrolix` / `mcp-trafficpeak`), built from one source tree via a build-time `MCP_BRAND` flag.
